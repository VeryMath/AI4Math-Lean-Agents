...
Use the canonical shared Skill layer at `skills/lean-formalization/SKILL.md` for Lean 4 formalization, proof repair, theorem transcription, `sorry` completion, Lean patch review, optional adapter-first Lean-specialist backend work, local Lean validation, and minimal failure handoff.
...
- Backend integration is adapter-first. Numina and Archon are recommended adapter candidates, not defaults or hard requirements; official Numina has a built-in recommended adapter recipe.
- Other Lean-specialist backends may be connected by the coding agent through `skills/lean-runtime/references/backend_adapter_checklist.md`; do not call any backend until deployment, readiness checks, invocation, validation, and failure triage are documented.
