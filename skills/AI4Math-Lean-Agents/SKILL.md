...
description: Use for interactive Lean 4 formal verification with reusable Lean/mathlib workspaces, direct coding-agent proof repair, theorem formalization, AutoADMM Strategy3 formalization, sorry completion, patch review, and minimal failure handoff.
---

# AI4Math Lean Agents

Use this skill when the user wants Lean 4 formalization, proof repair, theorem transcription, AutoADMM Strategy3 formalization, sorry completion, or review of a Lean patch. The active coding agent is the Lean agent: it asks clarifying questions, reads and edits Lean files, runs Lean/Lake checks, diagnoses errors, and iterates with the user.
...
3. For evolved adaptive-ADMM `update_rho` candidates, read `autoadmm-formalization/SKILL.md` and run its Strategy3 contract gate before rendering or checking Lean.
4. For standalone files, use or create `.ai4math/lean-workspace` only when a project context is needed.
...
- `autoadmm-formalization/scripts/autoadmm_formalize.py`: inspect, audit, render, verify, or certify Strategy3-safe AutoADMM candidates.
