...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="AI4Math direct Lean coding-agent skill CLI")
    sub = parser.add_subparsers(dest="command", required=True)

    env = sub.add_parser("env", help="Inspect direct Lean agent environment")
    add_common(env)
    ...
    batch = sub.add_parser("batch", help="Prepare a direct coding-agent folder task")
    add_common(batch)
